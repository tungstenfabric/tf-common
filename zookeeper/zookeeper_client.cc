...
void zk_watch(zhandle_t *zh, int type,
     ...
     ZOO_LOG(DEBUG, "Callback from Zookeeper zk_watch called with type = " <<type<< " state = " << state);
    ...
    std::string path1 = "/analytics-discovery-/" + g_vns_constants.COLLECTOR_DISCOVERY_SERVICE_NAME;
    path1 += "/" + self_hostname;
    ...
    if (!zoo_client__->CheckNodeExist(path1.c_str())) {
        ZOO_LOG(DEBUG, "zk_watch path = " << path1 << " doesn't exist. Creating node again in Zookeeper");
