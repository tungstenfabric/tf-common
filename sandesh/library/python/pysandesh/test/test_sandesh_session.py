...
from pysandesh.sandesh_session import SandeshSendQueue, SandeshSession
...
def create_fake_sandesh(size):
    ...
    len_width = len(sandesh_session._XML_SANDESH_OPEN) - \
        (len(sandesh_session._XML_SANDESH_OPEN_ATTR_LEN) +
         len(sandesh_session._XML_SANDESH_OPEN_END))
    sandesh = sandesh_session._XML_SANDESH_OPEN_ATTR_LEN + (
        str(size)).zfill(len_width) + sandesh_session._XML_SANDESH_OPEN_END + \
        ''.zfill(size - min_sandesh_len) + sandesh_session._XML_SANDESH_CLOSE
    return sandesh
...
class SandeshSessionTestHelper(sandesh_session.SandeshSession):
    ...
    def __init__(self):
        sandesh_session.SandeshSession.__init__(
...
class SandeshReaderTestHelper(sandesh_session.SandeshReader):
    def __init__(self, session):
        sandesh_session.SandeshReader.__init__(
            self, session, self._sandesh_msg_handler)
...
class SandeshReaderTest(unittest.TestCase):
    ...
    def test_read_msg(self):
        ...
        for i in range(len(msg_size_list)):
            self.assertEqual(
                msg_size_list[i],
                self._sandesh_reader.sandesh_msg_size_list[i])
    # end test_read_msg
...
class SandeshWriterTest(unittest.TestCase):
    ...
    def test_send_msg_more(self):
        ...
        exp_at_each_it = [
            ...
            dict(
                send_count=2,
                send_buf_cache_size=self._max_msg_size - 1),
            dict(
                send_count=3,
                send_buf_cache_size=0)]
        ...
        exp_at_end = [
            dict(
                msg=msg_info_list[0]['msg']),
            dict(
                msg=msg_info_list[1]['msg'] +
                msg_info_list[2]['msg'] +
                msg_info_list[3]['msg']),
            dict(
                msg=msg_info_list[4]['msg'] +
                msg_info_list[5]['msg'])]
        self.assertEqual(
            exp_at_each_it[len(exp_at_each_it) - 1]['send_count'],
            len(exp_at_end))
    ...
    def test_send_msg_all(self):
        ...
        for i in range(len(msg_info_list)):
            if msg_info_list[i]['send_all'] is True:
                self._sandesh_writer.send_sandesh(
                    msg_info_list[i]['msg'], False)
            else:
                self._sandesh_writer.send_sandesh(
                    msg_info_list[i]['msg'], True)
            self.assertEqual(exp_at_each_it[i]['send_count'],
                             len(self._sandesh_writer.get_send_buf_list()))
            self.assertEqual(exp_at_each_it[i]['send_buf_cache_size'],
                             len(self._sandesh_writer._send_buf_cache))
...
class SandeshSessionTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sandesh_instance.init_generator(
            'SandeshSessionTest', 'localhost', 'UT', 0, None,
            'context', -1, connect_to_collector=False)
    # end setUp
