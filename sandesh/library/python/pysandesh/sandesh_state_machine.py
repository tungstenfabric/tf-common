...
class SandeshStateMachine(object):
    ...
    def __init__(self, connection, logger, collectors, stats_collector):
        ...
        def _update_connection_state(e, status):
            ...
            ConnectionState.update(
                conn_type=ConnectionType.COLLECTOR,
                name='Collector',
                status=status,
    ...
    def on_session_event(self, session, event):
        if session is not self._session:
            self._logger.error(
                "Ignore session event [%d] received for old session" %
                (event))
            return
    ...
    def on_sandesh_ctrl_msg_receive(self, session, sandesh_ctrl, collector):
        if sandesh_ctrl.success:
            ...
            self._logger.error(
                "Negotiation with the Collector %s failed." %
                (collector))
            self._session.close()
    ...
    def _dequeue_event(self, event):
        ...
        if event.event == Event._EV_SANDESH_UVE_SEND:
            if self._fsm.current == State._ESTABLISHED or \
               self._fsm.current == State._CLIENT_INIT:
                self._connection.handle_sandesh_uve_msg(event.msg)
            else:
                self._connection.sandesh_instance().drop_tx_sandesh(
                    event.msg, SandeshTxDropReason.WrongClientSMState)
            ...
            self._logger.info(
                "Sandesh Client: Event[%s] => State[%s] -> State[%s]" %
