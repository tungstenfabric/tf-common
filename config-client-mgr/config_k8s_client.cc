...
void ConfigK8sClient::K8sToCassNameConversionInit()
{
    ...
    else
    {
        ...
        k8s_to_cass_name_conversion_["FloatingIP"] = "floating_ip";
    }
...
Value ConfigK8sClient::K8sJsonCreateRef(const std::string& ref_name, const Value* ref_info, Document& cass_dom)
{
    Value ref_array_val;
    ...
    {
        if (ref_name == "route_target_refs")
        {
            FixRouteTargetName(fq_name->value);
        }
...
bool ConfigK8sClient::FixRouteTargetName(const Value& rt)
{
    if (!rt.IsArray())
    {
        return false;
    ...
    auto fq_name_array = rt.GetArray();
    if (fq_name_array.Empty())
    {
        return false;
    }
    ...
    char* rt_name = NULL;
    for(auto i = fq_name_array.Begin(); i != fq_name_array.End(); ++i)
    {
        rt_name = const_cast<char*>(i->GetString());
    }
    for (char* rt_name_char = rt_name; *rt_name_char != '\0'; ++rt_name_char)
    {
        if (*rt_name_char == '-')
        {
            *rt_name_char = ':';
    ...
    map<string, Value>& ref_map)
{
    ...
    {
        Value ref_object = K8sJsonCreateRef(ref_name_str, &(ref->value), cass_dom);
    ...
    else if (ref->value.IsArray()) 
    {
        for (const Value* ref_info = ref->value.GetArray().Begin();
             ...
             ++ref_info)
        {
            Value ref_info_val = K8sJsonCreateRef(ref_name_str, ref_info, cass_dom);
...
bool ConfigK8sClient::K8sJsonConvert(
    const Document& dom, Document& cass_dom)
{
    if (!dom.IsObject()) {
        CONFIG_CLIENT_WARN(ConfigClientMgrWarning, "K8S SM: Document is not a JSON Object.");
        return false;
    }
    ...
    string uuid_string;
    if (uid == metadata->value.MemberEnd()) {
        CONFIG_CLIENT_WARN(ConfigClientMgrWarning, "K8S SM: Document missing uid.");
        return false;
    }
    if (!uid->value.IsString()) {
        CONFIG_CLIENT_WARN(ConfigClientMgrWarning, "K8S SM: Document uid is not a string.");
        return false;
    }
    uuid_string = uid->value.GetString();
    if (uuid_string.empty()) {
        CONFIG_CLIENT_WARN(ConfigClientMgrWarning, "K8S SM: Document uid is empty.");
        return false;
    }
    ...
    if (metadata != dom.MemberEnd() && annotations != metadata->value.MemberEnd() && !annotations->value.IsNull())
    {
    ...
    if (spec == dom.MemberEnd() || !spec->value.IsObject() || spec->value.IsNull()) {
        CONFIG_CLIENT_WARN(ConfigClientMgrWarning, "K8S SM: spec missing or is not an object.");
        return false;
    }
    auto fq_name = spec->value.MemberEnd();
    fq_name = spec->value.FindMember("fqName");
    if (fq_name == spec->value.MemberEnd()) {
        CONFIG_CLIENT_WARN(ConfigClientMgrWarning, "K8S SM: Object missing fqName.");
        return false;
    }
    ...
    if (spec != dom.MemberEnd() && !spec->value.IsNull()) {
            ...
            ++spec_member)
        {
            ...
            {
                if (type_val_str == "route_target")
                {
                    FixRouteTargetName(spec_member->value);
                }
                                         ...
                                         string value)
{
    ...
    Document cass_json;
    if (!K8sJsonConvert(dom, cass_json)) {
        // Conversion failed, ignore it.
        return;
    }
